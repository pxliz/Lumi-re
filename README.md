...
An automatic LED lighting system that turns a light on or off based on **light level** and **motion detection**, using a NodeMCU (ESP8266) programmed with the Arduino IDE.
...
The LED only turns on when both conditions are met: motion is detected and the room is dark enough. This avoids turning the light on during the day even if motion is detected, and avoids false triggers from light changes alone to save energy.

## Hysteresis logic

Using a single fixed brightness threshold causes the light to flicker on/off rapidly when ambient light hovers right around that value. To prevent this, the system uses **two separate thresholds** instead of one:

- Turn **ON** when the light level drops below **100** (LDR analog reading).
- Turn **OFF** only when the light level rises above **110**.

This creates a buffer zone between the two thresholds so the system doesn't oscillate rapidly near the switching point.
